...
sensor.set_framesize(sensor.QQVGA) #160x120 resolution for the camera sensor
...
while(True):
    ...
    img = sensor.snapshot()
    blob_stats = []
    blobs = img.find_blobs(threshold_list, pixels_threshold=300, area_threshold=200, merge=False)
    ...
    for blob_stat in blob_stats:
        if sensor.IOCTL_LEPTON_GET_FPA_TEMPERATURE < 534:
          ...
          tv.display("warning: %f" % sensor.ioctl(sensor.IOCTL_LEPTON_GET_FPA_TEMPERATURE))
          print("warning")
